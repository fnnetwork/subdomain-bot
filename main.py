...
cloudflare_configs = {
    'fnnetworkofficial.com': {
        'api_key': 'pjzwquCGEOh7JDETwgAESUjHonBIUd5rGykOPBbw',
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    keyboard = [[InlineKeyboardButton("Aᴅᴅ Dᴏᴍᴀɪɴ Bʏ Fɴ", callback_data='adddomain')],
                [InlineKeyboardButton("Cʟᴇᴀʀ Dᴏᴍᴀɪɴ", callback_data='cleardomain')]]
...
async def adddomain(update: Update, context: CallbackContext) -> int:
    ...
    if len(user_subdomains[user_id]) < 5:
        ...
        await update.message.reply_text(
            f"🔔 Nᴇᴡ Sᴜʙ-Dᴏᴍᴀɪɴ Cʀᴇᴀᴛᴇᴅ!\n\n"
            f"👤 Aᴅᴍɪɴ: @{ADMIN_USERNAME}\n\n"
            f"🌐 Sᴜʙᴅᴏᴍᴀɪɴ: {new_subdomain}\n"
            f"📍 Iᴘ Aᴅᴅʀᴇss: {ip_address}"
        )
        # Notify the admin
        await context.bot.send_message(
            ...
            text=(
                f"🔔 Nᴇᴡ Sᴜʙ-Dᴏᴍᴀɪɴ Cʀᴇᴀᴛᴇᴅ!\n\n"
                f"👤 Usᴇʀ: @{user.username if user.username else user.full_name}\n"
                f"🆔 Usᴇʀ Iᴅ: {user_id}\n"
                f"🌐 Sᴜʙᴅᴏᴍᴀɪɴ: {new_subdomain}\n"
                f"📍 Iᴘ Aᴅᴅʀᴇss: {ip_address}"
            )
...
def generate_random_subdomain(domain):
    ...
    return f"{random_text}.electraXfn.{domain}"
